...
from skylinetl._updates import ChannelState, Entity, EntityType, SessionState
from skylinetl.errors import RPCError
from skylinetl.errors.rpcerrorlist import TopicDeletedError
from skylinetl.hints import EntityLike
from skylinetl.network import MTProtoSender
from skylinetl.tl import functions
from skylinetl.tl.alltlobjects import LAYER
from skylinetl.tl.functions.channels import GetFullChannelRequest
from skylinetl.tl.functions.users import GetFullUserRequest
...
class CustomTelegramClient(TelegramClient):
    ...
    async def get_entity(
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._skyline_entity_cache
            and (
                not exp
                or self._skyline_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._skyline_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._skyline_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
        ...
        if resolved_perms:
            ...
            self._skyline_perms_cache.setdefault(hashable_entity, {})[hashable_user] = (
                cache_record
            )
    ...
    async def get_fullchannel(
        ...
        if (
            ...
            and not self._skyline_fullchannel_cache[hashable_entity].expired
    ...
    async def get_fulluser(
        ...
        if (
            ...
            and not self._skyline_fulluser_cache[hashable_entity].expired
