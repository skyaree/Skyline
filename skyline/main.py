...
from skylinetl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from skylinetl.password import compute_check
from skylinetl.sessions import MemorySession, SQLiteSession
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("skyline-") or f.startswith("hikka-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            logging.debug(
                ...
                client.tg_id,
                client.skyline_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
