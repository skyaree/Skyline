...
from skylinetl.errors.rpcbaseerrors import (
    ServerError,
    RPCError
)
from . import utils
...
def getlines(filename: str, module_globals=None) -> str:
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (module.startswith("skyline.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        if (
                            ...
                            value,
                            (skylinetl.TelegramClient, CustomTelegramClient),
                        ):
...
class TelegramLogsHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_skyline_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_skyline_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
...
def init():
    ...
    logging.getLogger("skylinetl").setLevel(logging.WARNING)
