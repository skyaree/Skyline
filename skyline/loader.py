...
from skylinetl.tl.tlobject import TLObject
...
def patched_import(name: str, *args, **kwargs):
    if name.startswith("telethon"):
        return native_import("skylinetl" + name[8:], *args, **kwargs)
    elif name.startswith("hikkatl"):
        return native_import("skylinetl" + name[7:], *args, **kwargs)
    elif name.startswith("hikkalls"):
        return native_import(name, *args, **kwargs)
    elif name.startswith("hikka"):
        return native_import("skyline" + name[5:], *args, **kwargs)
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _skyline_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?skyline$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.skyline_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.skyline_commands)
                inline_handlers.update(module.skyline_inline_handlers)
                callback_handlers.update(module.skyline_callback_handlers)
                watchers.extend(module.skyline_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.skyline_commands))
            )
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.skyline_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.skyline_watchers.values():
