...
class SkylineWebMod(loader.Module):
        ...
        if "OTHERHOST" in os.environ or "JAMHOST" in os.environ:
            ...
            if "LAVHOST" in os.environ:
                form = await self.inline.form(
                    ...
                    reply_markup={
                       ...
                       "url": await main.skyline.web.get_url(proxy_pass=False),
                    },
            ...
            if (
                ...
                try:
                    if not await self.inline.form(
                        ...
                        photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/skyline/web_interface.png",
                    ):
                        raise Exception
                except Exception:
                    await utils.answer(
            ...
            if not main.skyline.web:
                main.skyline.web = core.Web(
                    ...
                    api_token=main.skyline.api_token,
                    proxy=main.skyline.proxy,
                    connection=main.skyline.conn,
                )
                await main.skyline.web.add_loader(self._client, self.allmodules, self._db)
                await main.skyline.web.start_if_ready(
                    len(self.allclients),
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.skyline.api_token.ID,
            main.skyline.api_token.HASH,
            connection=main.skyline.conn,
            proxy=main.skyline.proxy,
