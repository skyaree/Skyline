...
class SkylineSettingsMod(loader.Module):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"skyline-logs", "silent-tags"}
                )
                or (
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        if callable(key):
            key()
            skylinetl.extensions.html.CUSTOM_EMOJIS = not main.get_config_key(
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._skyline_entity_cache)} cache records"
                )
                self._client._skyline_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._skyline_fulluser_cache)} cache records"
                )
                self._client._skyline_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._skyline_fullchannel_cache)} cache"
                    " records"
                )
                self._client._skyline_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._skyline_perms_cache)} cache records"
                self._client._skyline_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._skyline_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._skyline_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._skyline_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._skyline_fulluser_cache = {}
                self._client._skyline_fullchannel_cache = {}
                self._client.skyline_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._skyline_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._skyline_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._skyline_fullchannel_cache)} records\nLoader"
